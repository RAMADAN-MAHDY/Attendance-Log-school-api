...
import Students from '../../schema/regstur.js'; // عدل حسب مسار الملف
  ...
  try {
    ...
    for (const grade of grades) {
      for (const classRoom of grade.classes) {
        ...
        await Students.insertMany(newStudents);
        console.log(`✔️ أُضيف 40 طالب في ${grade.grade} - ${classRoom}`);
      }
    }

    console.log("✅ تم إعادة إنشاء جميع الطلاب بنجاح");

  } catch (error) {
    console.error("❌ حصل خطأ أثناء إعادة إنشاء الطلاب:", error);
  }
};
